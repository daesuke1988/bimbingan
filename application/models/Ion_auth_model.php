...
class Ion_auth_model extends CI_Model {
     ...
     * @var array
     * */
    protected $_cache_functions = array();

    /**
     * caching of group and their access
     *
     * @var array
     * */
    public $_cache_user_in_access = array();
    ...
    public function __construct() {
        ...
        $this->user_email_column = $this->config->item('user_email', 'ion_auth');
        $this->username_column = $this->config->item('username', 'ion_auth');
    ...
    public function login($identity, $password, $remember = FALSE) {
        ...
        $query = $this->db->select($this->identity_column . ', username, email, id, password, active, last_login')
                //->where($this->identity_column, $this->db->escape_str($identity))
                ->where($this->user_email_column, $this->db->escape_str($identity))
                ->or_where($this->username_column, $this->db->escape_str($identity))
                ->limit(1)
                ->get($this->tables['users']);
        ...
        if ($query->num_rows() === 1) {
            ...
            if ($password === TRUE) {
                ...
                $this->limit(1);
                $this->where($this->tables['users'] . '.id', $user->id);
                $cek = $this->users()->row();
                
                $this->session->set_userdata('user_row', $cek);
                $this->session->set_userdata('resume_info', "");
                $this->session->set_userdata('user_num_rows', count($cek));
                
                //--- save user groups

                $users_groups = $this->db->select($this->tables['users_groups'] . '.' . $this->join['groups'] . ' as id, ' . $this->tables['groups'] . '.name, ' . $this->tables['groups'] . '.description')
                            ->where($this->tables['users_groups'] . '.' . $this->join['users'], $user->id)
                            ->join($this->tables['groups'], $this->tables['users_groups'] . '.' . $this->join['groups'] . '=' . $this->tables['groups'] . '.id')
                            ->get($this->tables['users_groups'])->result();

                $this->session->set_userdata('users_groups', $users_groups);
    ...
    public function logiin($identity, $password) {
        $this->trigger_events('pre_login');
        ...
        $query = $this->db->select("mhs.*,users.id as id_user,users.id_fakultas,users.email,users.id_jurusan")->from("mhs")
                ->join("users","mhs.nim=users.username")->where("mhs.nim",$identity)->where("mhs.pinwali",$password)->get();
        ...
        if ($query->num_rows() === 1) {
            ...
            if (is_object($user)) {
                $profile = (object) array(
                    'id' => '00000'.$user->id,
                    'username' => $user->nim,
                    'first_name' => 'Wali Mahasiswa : '.$user->nama,
                    'last_name' =>'', 
                    'last_login' =>  date('H:i:s Y-m-d'), 
                    'email' =>$user->email, 
                    'id_fakultas' => $user->id_fakultas, 
                    'id_jurusan' => $user->id_jurusan, 
                    'user_id' => $user->id_user,
                );
                $this->session->set_userdata('user_row', $profile);
                $this->load->model('akademik_wali/akademik_wali_models');
                $info = $this->akademik_wali_models->dhs_mhs($user->nim);
                $this->session->set_userdata('resume_info', "Resume : <br>SKS yang sudah ditempuh : ".$info['sks_ditempuh']."<br>IPK : ".round($info['ipk'],2));
                $this->session->set_userdata('user_num_rows', count($profile));
                $users_groups = $this->db->select($this->tables['groups'] . '.*')->where($this->tables['groups'] . ".name ilike 'wali'")->get($this->tables['groups'])->result();
                $this->session->set_userdata('users_groups', $users_groups);
                $this->set_session($profile);
                $this->trigger_events(array('post_login', 'post_login_successful'));
                $this->trigger_events('privilege');
        
                $user_id = $this->session->userdata('user_id');
                $records = $this->db->select('f.id, f.name')->from('groups g')
                ->join('privileges p', 'p.group_id = g.id', 'inner')
                ->join('functions f', 'p.function_id = f.id', 'inner')->where("g.name ilike '%wali%' ")->get()->result();
                $privileges = array();
                foreach ($records as $r) {
                    $privileges[$r->name] = $r->id;
                }
                $this->session->set_userdata('user_privileges', $privileges);
    ...
    public function get_users_groups($id = FALSE) {
        ...
        if($id){
            return $this->db->select($this->tables['users_groups'] . '.' . $this->join['groups'] . ' as id, ' . $this->tables['groups'] . '.name, ' . $this->tables['groups'] . '.desc')
                            ->where($this->tables['users_groups'] . '.' . $this->join['users'], $id)
                            ->join($this->tables['groups'], $this->tables['users_groups'] . '.' . $this->join['groups'] . '=' . $this->tables['groups'] . '.id')
                            ->get($this->tables['users_groups']);
        }else{
            $result = new UserGroupsObject($this);

            if(!$result->result()){

                $id = $this->session->userdata('id_user');

                $cek = $this->db->select($this->tables['users_groups'] . '.' . $this->join['groups'] . ' as id, ' . $this->tables['groups'] . '.name, ' . $this->tables['groups'] . '.desc')
                                ->where($this->tables['users_groups'] . '.' . $this->join['users'], $id)
                                ->join($this->tables['groups'], $this->tables['users_groups'] . '.' . $this->join['groups'] . '=' . $this->tables['groups'] . '.id')
                                ->get($this->tables['users_groups'])->result();

                $this->session->set_userdata('users_groups', $cek);
    ...
    public function privilege($function) {
        ...
        if(!$privileges){
            ...
            $this->db->join('privileges p', 'p.group_id = g.id', 'inner');
            $this->db->join('functions f', 'p.function_id = f.id', 'inner');
            $this->db->join('users_groups ug', 'ug.group_id = g.id', 'inner');
            $this->db->join('users u', 'ug.user_id = u.id', 'inner');
            $this->db->where('u.id', $user_id);
            //$this->db->where('f.name', $function);
        ...
        return array_key_exists($function, $privileges);

    }

    public function remove_group_from_access($function_ids = false, $group_id = false) {
        $this->trigger_events('remove_group_from_access');
        ...
        if (!empty($function_ids)) {
            if (!is_array($function_ids)) {
                $function_ids = array($function_ids);
            }

            foreach ($function_ids as $function_id) {
                $this->db->delete($this->tables['privileges'], array($this->join['functions'] => (int) $function_id, $this->join['groups'] => (int) $group_id));
                if (isset($this->_cache_group_in_access[$group_id]) && isset($this->_cache_group_in_access[$group_id][$function_id])) {
                    unset($this->_cache_group_in_access[$group_id][$function_id]);
                }

                $this->db->delete($this->tables['privileges'], array());
            }
        ...
        // otherwise remove user from all groups
            ...
            if ($return = $this->db->delete($this->tables['privileges'], array($this->join['group_id'] => (int) $group_id))) {
                $this->_cache_group_in_access[$group_id] = array();
            }
        }
        return $return;
    }
     ...
     *
     * @return bool
     * @author Banu Desi Antoro
     * */
    //public function add_group_to_access($group_id, $user_id=false)
    public function add_group_to_access($function_id, $group_id = false) {
        $this->trigger_events('add_group_to_access');

        //if no id was passed use the current users id
        //$group_id || $group_id = $this->session->userdata('user_id');
        //check if unique - num_rows() > 0 means row found
        if ($this->db->where(array($this->join['functions'] => (int) $function_id, $this->join['groups'] => (int) $group_id))->get($this->tables['privileges'])->num_rows()) {
            return false;
        } else
            return true;

        if ($return = $this->db->insert($this->tables['privileges'], array($this->join['functions'] => (int) $function_id, $this->join['groups'] => (int) $group_id))) {
            if (isset($this->_cache_access[$function_id])) {
                $function_name = $this->_cache_access[$function_id];
            }

            //belum selesai
            else {
                $function = $this->group($group_id)->result();
                $group_name = $group[0]->name;
                $this->_cache_groups[$group_id] = $group_name;
            }
            $this->_cache_user_in_group[$user_id][$group_id] = $group_name;
     ...
     * @author Banu Desi Antoro
     * */
    public function remove_from_access($function_ids = false, $group_id = false) {
        $this->trigger_events('remove_from_access');
        ...
        if (!empty($function_ids)) {
            ...
            foreach ($function_ids as $function_id) {
                $this->db->delete($this->tables['privileges'], array($this->join['functions'] => (int) $function_id, $this->join['groups'] => (int) $function_id));
                if (isset($this->_cache_user_in_access[$function_id]) && isset($this->_cache_user_in_access[$group_id][$function_id])) {
                    unset($this->_cache_user_in_access[$group_id][$function_id]);
                }
            }
        ...
        // otherwise remove user from all groups
            ...
            if ($return = $this->db->delete($this->tables['privileges'], array($this->join['groups'] => (int) $group_id))) {
                $this->_cache_user_in_group[$group_id] = array();
            }
        }
        return $return;
    }
     ...
     * @author Banu Desi Antoro
     * */
    public function add_to_access($function_id, $group_id = false) {
        $this->trigger_events('add_to_access');
        ...
        if ($return = $this->db->insert($this->tables['privileges'], array($this->join['functions'] => (int) $function_id, $this->join['groups'] => (int) $group_id))) {
            if (isset($this->_cache_groups[$function_id])) {
                $function_name = $this->_cache_functions[$function_id];
            } else {
                $function = $this->funct($function_id)->result();
                $function_name = $function[0]->name;
                $this->_cache_functions[$function_id] = $function_name;
            }
            $this->_cache_user_in_access[$group_id][$function_id] = $function_name;
        }
        return $return;
    }
     ...
     * @author Banu Desi Antoro
     * */
    public function funct($id = NULL) {
        $this->trigger_events('function');

        if (isset($id)) {
            $this->db->where($this->tables['functions'] . '.id', $id);
        }
    ...
    public function functions() {
        ...
        $this->response = $this->db->get($this->tables['functions']);
    ...
    public function get_menu() {
        ...
        $this->trigger_events('get_menu');
        $users_groups = $this->get_users_groups();
        $num_rows = $users_groups->num_rows();

        $functions_privileges = $this->session->userdata('functions_privileges');

        if(!$functions_privileges){
            if ($num_rows > 0) {
                $groups = 'p.group_id = ' . $users_groups->row(0)->id;
                for ($i = 2; $i < $num_rows; $i++) {
                    $groups = $groups . 'or p.group_id = ' . $users_groups->row($i)->id;
                }
                $groups = "and (" . $groups . ")";
            }else{
                $groups = "and (p.group_id =-1)";
            }

            $sql = "SELECT f.name as link, f.desc, f.menu, f.id,f.kode,f.icon FROM functions f INNER JOIN privileges p ON f.id = p.function_id WHERE f.menu = 't' ".$groups." group by f.id order by f.no_urut ASC";
            $functions_privileges = $this->db->query($sql)->result_array();
            $this->session->set_userdata('functions_privileges', $functions_privileges);
        }

        return $this->menu_lib->prepare_list_menu($functions_privileges);
    }

    public function is_group($group) {
        $groups = $this->get_users_groups()->result();

        foreach ($groups as $g) {
            if ($group == $g->id || $group == $g->name) {
                return true;
            }
        }
        return false;
...
class UserObject
{
    ...
    function row()
    {
        return $this->ci->session->userdata('user_row');
    }

    function num_rows(){
        return $this->ci->session->userdata('user_num_rows');
    }

}

class UserGroupsObject
{
    private $ci;
    ...
    function row()
    {
        $users_groups = $this->ci->session->userdata('users_groups');
        return ($users_groups ? $users_groups[0] : array());
    }

    function result()
    {
        $users_groups = $this->ci->session->userdata('users_groups');
        return $users_groups ? $users_groups : array();
    }

    function num_rows(){
        $users_groups = $this->ci->session->userdata('users_groups');
        return $users_groups ? count($users_groups) : 0;
    }    
}
