...
if (!function_exists('_render_manage_priviledge')) {
    ...
    function _render_function_manage_priviledge($list_data, $CI = false)
    {
        ...
        foreach ($list_data as $kd => $row_list) {
            ...
            echo '<div style="display: block;" class="tree-folder">
                    <div class="tree-folder-header" title="' . $row_list['name'] . '">
                    ' . form_checkbox(array('name' => 'privileges[]', 'id' => $row_list['id'], 'value' => $row_list['id'], 'checked' => $access, 'class' => 'chk-col-blue')) . '
                    <label for="' . $row_list['id'] . '">' . $row_list['desc'] . '</label>
                    </div>';
            if (isset($row_list['child'])) {
    ...
    function _render_menu_for_layout($list_data, $CI = false)
    {
        ...
        foreach ($list_data as $kd => $row_list) {
            ...
            if (isset($row_list['child']) && !empty($row_list['child'])) {
                $__spanhaschild = '<span class="pull-right-container"> <i class="fa fa-angle-right pull-right"></i></span>';
            ...
            if (isset($row_list['child']) && !empty($row_list['child'])) {
                if (isset($__liactive)) {
                    echo '<li class="treeview ' . $__liactive . '">
